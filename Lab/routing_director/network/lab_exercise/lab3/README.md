# Lab exercise 3, configuring Traffic engineering using LSP
## Objective

In this lab, we are going to configure L3VPN to use different LSP accross the network 

The following configuration will be done on the network devices: 
1. Transport Class

        set routing-options route-distinguisher-id 10.100.255.1
        set routing-options router-id 10.100.255.1
        ...
        set routing-options transport-class name silver color 200
        set routing-options forwarding-table export LB

2. LSP with BGP transport class

        set protocols mpls label-switched-path lsp1_via_p5 to 10.100.255.3
        set protocols mpls label-switched-path lsp1_via_p5 primary via_p5
        set protocols mpls label-switched-path lsp1_via_p5 transport-class gold
        set protocols mpls label-switched-path lsp2 to 10.100.255.3
        set protocols mpls label-switched-path lsp2 transport-class silver
        set protocols mpls path via_p5 10.100.0.1
        set protocols mpls path via_p5 10.100.255.15

3. assign map routing instances to different transport class

        set policy-options policy-statement cust1 term 1 then community add rt-cust1
        set policy-options policy-statement cust1 term 1 then community add gold
        set policy-options policy-statement cust1 term 1 then accept
        set policy-options policy-statement cust2 term 1 then community add rt-cust2
        set policy-options policy-statement cust2 term 1 then community add silver
        set policy-options policy-statement cust2 term 1 then accept
        set policy-options community gold members color:0:100
        set policy-options community rt-cust1 members target:25000:1001
        set policy-options community rt-cust2 members target:25000:2001
        set policy-options community silver members color:0:200
        set routing-instances cust1 vrf-export cust1
        set routing-instances cust1 vrf-target target:25000:1001
        set routing-instances cust2 vrf-export cust2
        set routing-instances cust2 vrf-target target:25000:2001
